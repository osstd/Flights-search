...
def input_city(number):
    ...
    cities = [{'city': '', 'iataCode': '', 'result': ""} for _ in range(number)]
    if request.method == 'POST':
...
async def flight_search(from_time, to_time, nights_from, nights_to, stops, currency):
    origin_city['iataCode'], err = await FlightSearch.check_code(origin_city['city'])
    results = []
...
async def process_city(city, from_time, to_time, nights_from, nights_to, stops, currency):
    ...
    city['iataCode'], err = await FlightSearch.check_code(city['city'])
    if not city['iataCode']:
        return f"{city['city'].capitalize()}:\nNo IATA code was found for this city, please check the name and try " \
               f"again"
    elif err:
        return f"{city['city'].capitalize()}:\nError, {err}"
    flight, err = await FlightSearch.find_flights(origin_city['iataCode'], city['iataCode'], from_time, to_time,
    ...
    if not flight:
        ...
        return f"{city['city'].capitalize()},\nfound a route!\nOnly {currency}{flight.price} to fly " \
